...
| 模块 | 关系 |
|---|---|
| `runtime/autonomous_loop.py` + `runtime/dispatch_guard.py` + `selector/filters.py` | **已集成（2026-07-07，三轮 review 修复后）**：所有输入派发（动作 `runner.run`、流内终点点击、ESC 恢复）统一经 **`DispatchGuard` 单一 seam** 判定（kill switch + blocking hold + allowlist），新增派发路径天然被守卫；`allowed_action_types` 在 `CandidateFilter` 层生效但**必须显式 `honor_runbook_hints=True`**（Advisor/replay 链默认免疫 runbook hints）；backstop 同时覆盖派发拦截与 **selector 饥饿**（候选全被 allowlist 拒绝），达阈值发 `action_filter_stuck`；**runbook 完成 = blocking hold + 一次性 `runbook_completed` escalation（HUMAN）**，循环 idle 不再空转最终阶段；多步动作流中间求值 **`allow_transition=False`**（hold/abort 保持生效，阶段切换与落盘推迟到 tick 边界，绝不在动作飞行中换阶段）；**verifier 观察后、recovery 之前再刷新一次守卫**（verifier 会重新截图改 state，刷新后若 abort/hold 浮现则抑制 ESC）；各次求值的 escalation/transition **合并记录**（保留更强的 blocking hold，不把真实 hold 重标为 `transition_deferred`），去重键取 details 的**稳定判别子集**（`checked`/`missing_metrics`/触发条件身份），排除 `abort_result`/`exit_result`/`entry_result` 全量 dump——同阶段不同盲区仍视为不同事件，但持续 abort 期间非触发条件指标闪断不会误判为新事件泛洪；kill switch 冻结游标、dry-run 不落盘、escalation edge-triggered、外部删除的状态文件每 tick stat 检测并重建、未知 gate 确认只告警一次。入口 `app.autonomous --runbook`（持单实例 flock，双开直接报错），操作者确认 `app.runbook_gate`（必须能加载 runbook 以盖 season 戳，拒绝无戳确认；show 按赛季标注并标记 IGNORED 条目），CLI 路径参数全部 `expanduser` |
