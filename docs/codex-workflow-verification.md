...
| Desktop Advisor browser smoke | `docs/advisor-browser-smoke.md` | 文件存在；包含启动命令、browser checklist、通过标准、失败记录格式、推荐验证组合 |
| qa-agent MCP connector | `docs/qa-agent-mcp-connector.md` | 文件存在；包含 stdio 启动命令、3 个工具、connector 配置草案、MCP client 验证标准 |
...
- Browser smoke must use `$browser` against the local Advisor URL.
