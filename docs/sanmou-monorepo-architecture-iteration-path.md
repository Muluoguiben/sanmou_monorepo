...
32. [x] Desktop evidence/degraded 展示：桌面端 summary 面板优先渲染 `structured_evidence`，展示 `execution_blocked_reason`，并在无结构化证据或动作被 block 时用 `evidence-degraded` 状态标注“证据不足”；MCP Desktop gate 已验证该展示门槛通过。
33. [x] MCP live trace terminal source semantic validation：`live_trace_fixture` 不能只靠 trace 文件存在和人工填的 `verification_record` 通过；MCP 现在会读取 JSON/JSONL trace，要求 `selected_action.action_type`、`execution.status`、`execution.summary.target_key`、`terminal_for_verifier=true` 与 `verification.post_action_verifier.status=verified` 同时匹配，错误 trace 或失败 verifier 会继续保留 `low_risk_terminal_source_review` blocker。
34. [x] MCP terminal source evidence templates：`next_source_requirements` 现在为每个缺失低风险动作输出 `pr5_real_screenshot_fixture` / `live_trace_fixture` 两套 `terminal_source_evidence_templates`，`capture_plan.actions` 同时输出可直接补入 `advisor_fixture_expectations.json` 的 expectation patch skeleton，降低真实终态采样和 review 的字段歧义。
...
47. [x] MCP terminal source preflight skeleton：`capture_plan.actions` 现在输出 `advisor_fixture_manifest_target` 和两种 accepted source kind 的 `preflight_tool_calls`，真实采样后可直接把截图/trace/review 字段替换进 MCP 参数并跑 `advisor_terminal_source_evidence_eval`，再写回 golden expectation manifest。
48. [x] Terminal source preflight CLI：新增 `qa_agent.app.advisor_terminal_source_preflight`，真实采样后可用 `--action-type` + `--evidence-json` 在本地/CI 复用 MCP review 逻辑；证据 ready 返回 0，未 ready 返回 2 并输出 missing evidence、patch hints 与 manifest patch。
49. [x] Terminal source batch preflight gate：`qa_agent.app.advisor_terminal_source_preflight --batch-json` 现在可一次性校验三类低风险 terminal source evidence，默认要求 `claim_chapter_reward`、`recruit_soldiers`、`upgrade_building` 全部 accepted；漏一类或任一证据未 ready 都返回 2，避免只补单条 source 就误判 Architecture closure。
50. [x] Terminal source privacy review gate：`terminal_source_evidence` 现在必须带 `privacy_review.status=approved`、privacy reviewer/time、截图 scope、脱敏状态、账号/聊天/支付敏感字段 false 与 `approved_for_repo_storage=true`，否则即使 screenshot/trace/verifier 都匹配也不能解除低风险 source blocker；capture plan、evidence templates、preflight patch hints 和 batch CLI 均同步输出该字段，避免把含账号信息的原始客户端截图提交为 closure 证据。
51. [ ] 三个低风险动作的完整 UI flow，继续补 claim/recruit 面板打开、数量/确认序列；三类动作仍需真实截图终态样本和 post-action delta 样本补强。
