...
- [ ] 低风险 action handlers：三个低风险动作从 `pending` 推进到真实 UI flow，动作失败必须 block/recover，不允许继续连点。2026-05-30 已落地语义 bbox dispatch、dispatch 前 semantic-target gate、post-action verifier 与 immediate recovery slice：当 PR-4/PR-5 感知输出 validated button bbox 时，`claim_chapter_reward`、`recruit_soldiers`、`upgrade_building` 可派发一次 allowlisted click；缺失、disabled 或非法 bbox 会在 `UIActionRunner` 直接 `semantic_target_gate` block，不再落入未校准点击路径；通过 gate 的 dispatch 会把 `semantic_target_gate` allow verdict 写入 execution summary 和 `AutonomousLoop` act trace，并由 replay/MCP 的 `runtime_dispatch` dry result 验证；PR5 golden replay 已锁住 claim/recruit 缺可信 bbox 必须 block、upgrade 真实入口 bbox 可 dispatch；`AutonomousLoop` 会在点击后重新 observe 并验证 expected delta，验证失败会停止为 `failed`、同 tick 发送一次 ESC recovery，并把 recovery/input trace 写入 trace。`upgrade_building` 已追加保守两段式入口流：`upgrade_button` 点击后必须重新 observe，并由同一 `action_id` 的 terminal action 消费 `upgrade_dialog.confirm_button` 后才进入 verifier；PR-21/PR-22 追加三类低风险 terminal replay fixture，证明已观测终态按钮时会 dispatch 到 verifier-ready 终态。MCP `pr5_low_risk_terminal_dispatch_coverage` 现在要求 status ok 且 `terminal_for_verifier=true` 才算低风险终态覆盖；`low_risk_verifier_readiness` 的 terminal blocker 已清零，但 `low_risk_terminal_source_review` 明确这些终态覆盖仍是 runtime-state fixture，不足以完成 Architecture closure。claim/recruit 的打开面板、数量/确认序列仍待补齐，三类动作仍需真实截图终态样本与 post-action delta 样本补强。
...
- [ ] `claim_chapter_reward` flow：打开章节面板 → 定位可领取奖励 → 点击领取/确认 → 返回稳定页面；非 dry-run 执行成功时返回 `ok`。2026-05-30 已支持已观测 `progress.chapter_claim_button` bbox 的单次安全点击，并在点击后验证 `progress.chapter_claimable=false`；缺 bbox、disabled bbox 或非法 bbox 会在 dispatch 前 `semantic_target_gate` block，通过 gate 的 dispatch 会在 execution/trace 中保留 allow verdict，验证不通过时不会继续连点并触发一次 ESC recovery。
...
- [ ] `recruit_soldiers` flow：打开征兵面板 → 选择可征兵队伍/数量 → 确认征兵或安全退出；遇到资源不足、队伍 busy、未知弹窗时不得重复点击。2026-05-30 已支持已观测 `teams[*].recruit_button` bbox 的单次安全点击，并在点击后验证兵力、倒计时或预备兵 delta；缺 bbox、disabled bbox 或非法 bbox 会在 dispatch 前 `semantic_target_gate` block，通过 gate 的 dispatch 会在 execution/trace 中保留 allow verdict，验证失败会触发一次 ESC recovery，完整数量/确认流程仍待补。
...
- [ ] `upgrade_building` low-risk flow：只对白名单低风险建筑执行升级；完整建筑面板定位与失败恢复样本仍待补。2026-05-30 已支持在已观测 `upgrade_button` bbox 时先点击升级入口，重新 observe 后必须得到同类 `upgrade_building` terminal action 且 `city.upgrade_dialog.confirm_button` 建筑名匹配，才派发确认点击并验证建筑等级或木材 delta；入口按钮和确认按钮都会先过 `semantic_target_gate`，通过 gate 的 dispatch 会在 execution/trace 中保留 allow verdict，中间观察、同类动作或最终 verifier 任一失败都会停止并触发 recovery，不继续连点。PR5 建筑真实截图 fixture 已断言 `city.buildings[*].upgrade_button` 经 deriver 进入 selected action，并以 `city.buildings` 作为结构化 evidence。
