...
> Last updated: 2026-05-30 (低风险闭环继续推进：claim/recruit/upgrade 已有 semantic bbox dispatch；`UIActionRunner` 会在 dispatch 前二次校验低风险动作是否携带 visible+enabled 且 0-1000 合法的 semantic bbox，缺失、禁用或越界时直接 `semantic_target_gate` block，不伪造 PR5 claim/recruit bbox；通过 gate 的真实 dispatch 也会把 `semantic_target_gate` allow verdict 写入 `ExecutionResult.summary` 与 `AutonomousLoop` trace；PR5 replay 现在输出低风险 `runtime_dispatch` dry result，qa-agent MCP `advisor_fixture_eval` / `advisor_golden_replay_status` 同时暴露 `dispatch_gate`、`pr12_runtime_dispatch_coverage`、`attention_reasons`、`low_risk_verifier_readiness` 与 fixture-level `low_risk_readiness`；MCP status 已把 PR5 action/evidence/confidence/dispatch/terminal-dispatch locked fields 缺失纳入 `attention`；新增 `pr20_upgrade_confirm_terminal_state.json` 覆盖 `upgrade_dialog.confirm_button` 终态 dispatch，`upgrade_building` 已进入 low-risk `ready_actions`，当前完整 replay 唯一结构化 blocker 仍是 `low_risk_terminal_dispatch_missing`，但剩余动作为 `claim_chapter_reward` 与 `recruit_soldiers`；`include_fixture_results=false` 会以 `fixture_replay_not_run` 返回 `attention`，不能当成健康证明；单 fixture 可区分 claim/recruit 卡在 semantic gate、PR5 upgrade 卡在非终态 `open_upgrade_dialog`、PR20 upgrade confirm 已 terminal-ready，并输出下一张 required fixture：chapter claim button、recruit button terminal fixtures；`AutonomousLoop` 会在成功点击后重新 observe 并执行 post-action verifier，失败会标记 `failed` + `recovery_required` 并同 tick 触发一次 ESC recovery；MCP `architecture_iteration_closure_gate` 现将 ADR/路线文档、replay、PR5/PR6 gates 与 low-risk readiness 汇总为单一收口门槛，当前仍因 claim/recruit terminal dispatch 缺口保持 `attention`；claim/recruit 完整多步 UI flow 仍待补)
