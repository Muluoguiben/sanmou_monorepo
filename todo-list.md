...
- [x] Bilibili「三谋开荒」100 视频 discovery run（2026-05-17）：用 `qa_agent.app.discover_bilibili` 搜索 `三谋开荒` / `三国谋定天下开荒` / `三谋开荒攻略`，生成 100 个候选、20 个 batch，并由 subagents 每组 5 个视频跑 `fetch_bilibili_bundle -> run_video_pipeline --extractor heuristic`；产物落在 `packages/qa-agent/ingestion/video_discovery/run-2026-05-17/summary.md` 与 `review-queue.yaml`。当前未配置 `BILIBILI_COOKIE`，100/100 为 metadata-only，只能作为人工 review 队列，不能直接 publish 正式知识库。
- [ ] Bilibili high-priority review queue 二次沉淀：对 `run-2026-05-17/review-queue.yaml` 中 high-priority 视频，配置 `BILIBILI_COOKIE` 后重跑 `--asr-fallback` / `--with-frames --enrich-frames`，只把有字幕、ASR、关键帧或人工复核证据的条目 publish 到正式 `knowledge_sources/`。
