...
> Last updated: 2026-05-30 (低风险 action handler 语义 bbox dispatch slice：claim/recruit/upgrade 可在已观测 validated button bbox 时派发一次 allowlisted click；完整面板序列、后置 observe/verifier 与 recovery 仍是下一段重点)
...
- [ ] 低风险 action handlers：三个低风险动作从 `pending` 推进到真实 UI flow，动作失败必须 block/recover，不允许继续连点。2026-05-30 已落地语义 bbox dispatch slice：当 PR-4/PR-5 感知输出 validated button bbox 时，`claim_chapter_reward`、`recruit_soldiers`、`upgrade_building` 可派发一次 allowlisted click；打开面板、确认序列、后置 observe/verifier 与 recovery 仍待补齐。
...
- [ ] `claim_chapter_reward` flow：打开章节面板 → 定位可领取奖励 → 点击领取/确认 → 返回稳定页面；非 dry-run 执行成功时返回 `ok`。2026-05-30 已支持已观测 `progress.chapter_claim_button` bbox 的单次安全点击，缺 bbox 时仍 pending。
...
- [ ] `recruit_soldiers` flow：打开征兵面板 → 选择可征兵队伍/数量 → 确认征兵或安全退出；遇到资源不足、队伍 busy、未知弹窗时不得重复点击。2026-05-30 已支持已观测 `teams[*].recruit_button` bbox 的单次安全点击，完整数量/确认流程仍待补。
...
- [ ] `upgrade_building` low-risk flow：只对白名单低风险建筑执行升级；真实 UI flow、确认框序列与失败恢复仍待接入。2026-05-30 已支持在已打开且建筑名匹配的 `city.upgrade_dialog.confirm_button` bbox 上派发一次安全确认点击；打开建筑面板与后置验证仍待补。
