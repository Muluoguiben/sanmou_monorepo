...
from qa_agent.ingestion.client_evidence_bundle import (
    build_client_evidence_bundle,
    write_client_evidence_bundle,
)


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a repo-relative NSLG client offline evidence bundle for qa-agent import planning."
    )
    parser.add_argument("--repo-root", default=".", help="qa-agent package root.")
    parser.add_argument("--output", required=True, help="YAML bundle output path.")
    parser.add_argument("--source-id", default="nslg-client-offline-bundle")
    parser.add_argument("--package-manifest", default=None)
    ...
    parser.add_argument("--nep2-provenance-closures", default=None)
...
def main() -> None:
    ...
    bundle = build_client_evidence_bundle(
        ...
        package_manifest_path=Path(args.package_manifest) if args.package_manifest else None,
        ...
        lua_crypto_evidence_path=Path(args.lua_crypto_evidence) if args.lua_crypto_evidence else None,
        ...
        nep2_luascripts_evidence_path=(
            Path(args.nep2_luascripts_evidence) if args.nep2_luascripts_evidence else None
        ),
        nep2_provenance_closures_path=(
            Path(args.nep2_provenance_closures) if args.nep2_provenance_closures else None
        ),
        gameassembly_route_trace_path=(
            Path(args.gameassembly_route_trace) if args.gameassembly_route_trace else None
        ),
        ...
        runtime_init_route_path=Path(args.runtime_init_route) if args.runtime_init_route else None,
        ...
        gameassembly_resolver_trace_path=(
            Path(args.gameassembly_resolver_trace) if args.gameassembly_resolver_trace else None
        ),
        gameassembly_resolver_caller_trace_path=(
            Path(args.gameassembly_resolver_caller_trace)
            if args.gameassembly_resolver_caller_trace
            else None
        ),
        decoded_hero_audit_path=Path(args.decoded_hero_audit) if args.decoded_hero_audit else None,
    )
    write_client_evidence_bundle(bundle, Path(args.output))
    print(
        json.dumps(
            {
                "output": args.output,
                ...
                "evidence_ref_count": bundle.evidence_ref_count,
                "safe_for_publish": bundle.import_readiness.get("safe_for_publish"),
                "normalized_staging_entries": bundle.import_readiness.get(
                    "normalized_staging_entries"
                ),
                "decoder_target_artifacts": bundle.import_readiness.get(
