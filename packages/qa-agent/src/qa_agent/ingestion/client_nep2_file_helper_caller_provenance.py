...
class Nep2FileHelperInputArtifact(BaseModel):
    file_name: str = Field(min_length=1)
    sha256: str = ""
    missing: bool = False


class Nep2FileHelperCallerProvenanceReport(BaseModel):
    ...
    round: int = 0
    slice: str = ""
    input_artifacts: list[Nep2FileHelperInputArtifact] = Field(default_factory=list)
...
def build_nep2_file_helper_caller_provenance_report(
    *,
    input_path: Path,
    source_id: str,
    generated_at: datetime | None = None,
) -> Nep2FileHelperCallerProvenanceReport:
    generated_at = generated_at or datetime.now(timezone.utc)
    data = json.loads(input_path.read_text(encoding="utf-8"))
    conclusion = _safe_map(data.get("route_conclusion"))
    return Nep2FileHelperCallerProvenanceReport(
        source_id=source_id,
        ...
        guardrails=[
            "offline/static NEP2 direct-call graph scan only; no live instrumentation, account data, or online protocol data is included",
            "absolute local paths are not persisted; only file names, hashes, RVAs, counts, and sanitized summaries are stored",
            "file-helper evidence is route-closure evidence only and requires payload-buffer ownership before decoder promotion",
            "no decoded global-metadata, LuaScripts payload, or gameplay knowledge is promoted from this artifact",
        ],
    )


def write_nep2_file_helper_caller_provenance_report(
    report: Nep2FileHelperCallerProvenanceReport,
    output_path: Path,
) -> None:
...
def _input_artifact(raw: dict[str, Any]) -> Nep2FileHelperInputArtifact:
    file_name = str(raw.get("file_name") or "unknown").replace("\\", "/")
    return Nep2FileHelperInputArtifact(
...
def _target_summary(raw: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "round186_verdict": str(raw.get("round186_verdict") or ""),
        "helper_seed": str(raw.get("helper_seed") or ""),
        ...
        "payload_keyword_refs": [
            _keyword_summary(item) for item in raw.get("payload_keyword_refs") or []
        ][:24],
        ...
        "paths_to_helper_seed": [
            _path_summary(item) for item in raw.get("paths_to_helper_seed") or []
        ][:12],
        "paths_from_helper_seed": [
            _path_summary(item) for item in raw.get("paths_from_helper_seed") or []
        ][:12],
...
def _path_summary(raw: Any) -> dict[str, Any]:
    if not isinstance(raw, dict):
        return {}
    return {
        ...
        "path_length": len(raw.get("path") or []),
    }
