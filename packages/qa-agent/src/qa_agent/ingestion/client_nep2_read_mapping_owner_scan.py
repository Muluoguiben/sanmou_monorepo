...
class Nep2ReadMappingInputArtifact(BaseModel):
    file_name: str = Field(min_length=1)
    sha256: str = ""
    missing: bool = False


class Nep2ReadMappingOwnerScanReport(BaseModel):
    ...
    round: int = 0
    slice: str = ""
    input_artifacts: list[Nep2ReadMappingInputArtifact] = Field(default_factory=list)
    nep2_file: dict[str, Any] = Field(default_factory=dict)
    import_class_counts: dict[str, int] = Field(default_factory=dict)
    counts: dict[str, int] = Field(default_factory=dict)
    verdict_counts: dict[str, int] = Field(default_factory=dict)
    read_mapping_import_owner_counts: dict[str, int] = Field(default_factory=dict)
...
def build_nep2_read_mapping_owner_scan_report(
    *,
    input_path: Path,
    source_id: str,
    generated_at: datetime | None = None,
) -> Nep2ReadMappingOwnerScanReport:
    generated_at = generated_at or datetime.now(timezone.utc)
    data = json.loads(input_path.read_text(encoding="utf-8"))
    conclusion = _safe_map(data.get("route_conclusion"))
    return Nep2ReadMappingOwnerScanReport(
        source_id=source_id,
        ...
        nep2_file=_nep2_file_summary(_safe_map(data.get("nep2_file"))),
        import_class_counts=_int_dict(_safe_map(data.get("import_class_counts"))),
        counts=_counts(_safe_map(data.get("counts")), conclusion),
        verdict_counts=_int_dict(_safe_map(data.get("verdict_counts"))),
        read_mapping_import_owner_counts=_int_dict(
            _safe_map(data.get("read_mapping_import_owner_counts"))
        ),
        owners=[
            _owner_summary(item) for item in data.get("owners") or [] if isinstance(item, dict)
        ],
        ...
        guardrails=[
            "offline/static NEP2 import-owner scan only; no live instrumentation, account data, or online protocol data is included",
            "absolute local paths are not persisted; only file names, hashes, target RVAs, counts, and sanitized owner summaries are stored",
            "read/mapping import ownership without metadata/LuaScripts/protected-payload provenance is not decoder proof",
...
def write_nep2_read_mapping_owner_scan_report(
    report: Nep2ReadMappingOwnerScanReport,
    output_path: Path,
) -> None:
...
def _input_artifact(raw: dict[str, Any]) -> Nep2ReadMappingInputArtifact:
    file_name = str(raw.get("file_name") or "unknown").replace("\\", "/")
    return Nep2ReadMappingInputArtifact(
...
def _nep2_file_summary(raw: dict[str, Any]) -> dict[str, Any]:
    file_name = str(raw.get("file_name") or "NEP2.dll").replace("\\", "/")
    return {
        "file_name": Path(file_name).name,
        "sha256": str(raw.get("sha256") or ""),
    }
...
def _owner_summary(raw: dict[str, Any]) -> dict[str, Any]:
    return {
        "target_rva": str(raw.get("target_rva") or ""),
        "function": _safe_map(raw.get("function")),
        "verdict": str(raw.get("verdict") or ""),
        "owner_read_mapping_events": [
            _event_summary(item) for item in raw.get("owner_read_mapping_events") or []
        ][:24],
        "counts": _int_dict(_safe_map(raw.get("counts"))),
        "imports_seen": [str(item) for item in raw.get("imports_seen") or []],
        "read_mapping_imports_seen": [
            str(item) for item in raw.get("read_mapping_imports_seen") or []
        ],
        "provenance_labels": [str(item) for item in raw.get("provenance_labels") or []],
        "neighborhood_provenance_labels": [
            str(item) for item in raw.get("neighborhood_provenance_labels") or []
        ],
        "has_read_or_mapping_import": bool(raw.get("has_read_or_mapping_import")),
        "has_metadata_provenance": bool(raw.get("has_metadata_provenance")),
        "has_luascripts_provenance": bool(raw.get("has_luascripts_provenance")),
        "has_protected_payload_signal": bool(raw.get("has_protected_payload_signal")),
        ...
        "direct_callers": [
            _neighbor_summary(item) for item in raw.get("direct_callers") or []
        ][:24],
        "direct_callees": [
            _neighbor_summary(item) for item in raw.get("direct_callees") or []
        ][:24],
...
def _event_summary(raw: Any) -> dict[str, Any]:
    if not isinstance(raw, dict):
        return {}
    out: dict[str, Any] = {}
    for key in ("rva", "import", "import_name", "class", "text", "label", "value"):
        if key in raw:
            out[key] = raw.get(key)
    if raw.get("context"):
...
def _neighbor_summary(raw: Any) -> dict[str, Any]:
    if not isinstance(raw, dict):
        return {}
    signal = raw.get("signal") if isinstance(raw.get("signal"), dict) else {}
    return {
        "edge": _safe_map(raw.get("edge")),
        "signal": {
            "function": _safe_map(signal.get("function")),
            "counts": _int_dict(_safe_map(signal.get("counts"))),
            "imports_seen": [str(item) for item in signal.get("imports_seen") or []],
            "provenance_labels": [
                str(item) for item in signal.get("provenance_labels") or []
            ],
        },
    }
