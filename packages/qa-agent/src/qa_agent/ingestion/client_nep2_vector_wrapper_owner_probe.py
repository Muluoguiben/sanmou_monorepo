...
class Nep2VectorWrapperOwnerInputArtifact(BaseModel):
    file_name: str = Field(min_length=1)
    sha256: str = ""
    missing: bool = False


class Nep2VectorWrapperOwnerProbeReport(BaseModel):
    ...
    round: int = 0
    slice: str = ""
    input_artifacts: list[Nep2VectorWrapperOwnerInputArtifact] = Field(default_factory=list)
...
def build_nep2_vector_wrapper_owner_probe_report(
    *,
    input_path: Path,
    source_id: str,
    generated_at: datetime | None = None,
) -> Nep2VectorWrapperOwnerProbeReport:
    generated_at = generated_at or datetime.now(timezone.utc)
    data = json.loads(input_path.read_text(encoding="utf-8"))
    conclusion = _safe_map(data.get("route_conclusion"))
    return Nep2VectorWrapperOwnerProbeReport(
        source_id=source_id,
        ...
        wrappers=[_wrapper_summary(item) for item in data.get("wrappers") or []][:40],
        ...
        guardrails=[
            "offline/static NEP2 callgraph wrapper scan only; no live instrumentation, account data, or online protocol data is included",
            "absolute local paths are not persisted; only file names, hashes, RVAs, counts, and sanitized summaries are stored",
            "vector-wrapper call edges are route evidence only and require payload-buffer provenance before decoder promotion",
            "no decoded global-metadata, LuaScripts payload, or gameplay knowledge is promoted from this artifact",
        ],
    )


def write_nep2_vector_wrapper_owner_probe_report(
    report: Nep2VectorWrapperOwnerProbeReport,
    output_path: Path,
) -> None:
...
def _input_artifact(raw: dict[str, Any]) -> Nep2VectorWrapperOwnerInputArtifact:
    file_name = str(raw.get("file_name") or "unknown").replace("\\", "/")
    return Nep2VectorWrapperOwnerInputArtifact(
...
def _wrapper_summary(raw: Any) -> dict[str, Any]:
    if not isinstance(raw, dict):
        return {}
    return {
        "wrapper_rva": str(raw.get("wrapper_rva") or ""),
        ...
        "vector_calls": [_simple_map(_safe_map(item)) for item in raw.get("vector_calls") or []][:48],
        "file_helper_calls": [
            _simple_map(_safe_map(item)) for item in raw.get("file_helper_calls") or []
        ][:24],
        "parent_context_count": len(raw.get("parent_contexts") or []),
        "parent_contexts": [
            _simple_map(_safe_map(item)) for item in raw.get("parent_contexts") or []
        ][:16],
        "provenance_paths": [
